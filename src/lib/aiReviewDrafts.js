...
/**
 * Returns an array of `count` distinct review texts, or [] if generation
 * isn't possible (no API key, network/API failure, unusable response) — the
 * caller treats an empty result as "no drafts available", never an error
 * that blocks campaign creation.
 */
export async function generateReviewDrafts({ businessName, platform, notes, category, count }) {
  ...
  const prompt = [
    ...
    ``,
    `Respond with ONLY a JSON object: {"reviews": ["review 1", "review 2", ...]} with exactly ${n} items.`,
  ]
    .filter(Boolean)
    .join("\n");
