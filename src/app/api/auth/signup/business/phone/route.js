...
/**
 * Step 2 of phone+OTP SIGNUP for business owners. Role is fixed by this
 * route, never read from the body — same discipline as
 * src/lib/auth/signup.js's email/password signup.
 * Body: { phone, otp, name }.
 */
const schema = z
  .object({
...
export async function POST(request) {
  ...
  const result = await verifyPhoneOtpForSignup(
    { ...parsed.data, role: ROLES.BUSINESS_OWNER },
    request
  );
  if (!result.ok) {
